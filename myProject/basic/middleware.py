...
#         # check email rules withregex
#         # check dob rules withregex
#         # check password rules withregex
    ...
    def __call__(self,request):
        if(request.path == "/job1/"):
            ...
            if(medical_fit_result != 'True'):
                return JsonResponse({"error":"U are not medically fit to apply for this job role."},status=400)
        return self.get_response(request)    
